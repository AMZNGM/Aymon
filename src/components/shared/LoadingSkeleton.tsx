export default function LoadingSkeleton({ className = '', count = '0' }: { className?: string; count?: string }) {
  const gridCols = count === '1' ? 'grid-cols-1' : count === '2' ? 'grid-cols-2' : 'md:grid-cols-2'

  if (count === '0') {
    return (
      <div className="flex flex-col justify-center items-center gap-2 w-full h-dvh animate-pulse">
