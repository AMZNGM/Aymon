...
export default function Modal({ id, children, className }: { id: string; children: React.ReactNode; className?: string }) {
  return (
    <>
      ...
      <label
        htmlFor={id}
        className="z-9998 fixed inset-0 bg-bg/60 opacity-0 peer-checked/modal:opacity-100 transition-opacity duration-300 cursor-default pointer-events-none peer-checked/modal:pointer-events-auto"
      />
