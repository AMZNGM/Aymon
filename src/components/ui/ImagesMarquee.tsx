...
function MarqueeItem({
  ...
  const itemOffset = useTransform(baseOffset, (v: number) => {
    const position = (itemIndex * virtualPathLength) / itemsLength
    return wrap(0, virtualPathLength, v + position)
  })

  const opacity = useTransform(itemOffset, [0, 100, 100.01, virtualPathLength], [1, 1, 0, 0])
  const display = useTransform(itemOffset, (v: number) => (v <= 100 ? 'block' : 'none'))
  const offsetDistance = useTransform(itemOffset, (v: number) => `${v}%`)
