    ...
    <section className="relative w-full h-fit bg-text text-bg pt-12">
      <ImagesMarquee path={path} viewBox="50 50 1300 200" spacing={1.4}>
        {logos.map((logo, index) => (
          <div key={logo.firestoreId || index} className="w-24 hover:scale-125 duration-300 ease-in-out">
            <ImageIn
              src={logo.src}
              alt={`Worked With - ${index}`}
              priority={index < 5}
              sizes="100px"
              className="object-contain! brightness-0 pointer-events-none select-none"
