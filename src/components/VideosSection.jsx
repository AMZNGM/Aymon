...
const videos = [
  ...
  '/videos/randomVideos/feeling2.mp4',
...
export default function VideosSection() {
  ...
  return (
    <div ref={containerRef} className="relative gap-4 columns-1 sm:columns-2 lg:columns-3 max-md:px-1 py-12 pe-1">
      {videos.map((video, index) => (
        <motion.div
          ...
          viewport={{ once: true }}
          className="break-inside-avoid mb-4 cursor-grab active:cursor-grabbing"
        >
          <motion.article data-swapy-item={index} className="w-full">
            <video
              ...
              controls={false}
              src={video}
              className="w-full h-auto border border-bg/25 rounded-2xl pointer-events-none"
            />
          </motion.article>
        </motion.div>
      ))}
